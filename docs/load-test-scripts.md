...
```text
79.5k + 55k + 100k + 22.5k + 13.5k = 270.5k users
```
...
| Region set | Command |
|---|---|
| Full 270.5k current test | `./scripts/run-test-v6.sh --no-k8s --test 2` |
...
Sync all default flows for the 270.5k test:
...
- Current 270.5k command: `./scripts/run-test-v6.sh --no-k8s --test 2`.
