...
```text
79.5k + 55k + 55k + 22.5k + 13.5k = 225.5k users
```
...
|---|---|
| `./scripts/run-test-v6.sh --no-k8s --test 2` | Runs the current 225.5k-user regional test across Brazil, Turkey, Philippines, Saudi, and Egypt. Keeps dstat on, skips Kubernetes metrics. |
| `./scripts/run-test-v6.sh --no-k8s --no-dstat --test 2` | Runs the same 225.5k-user test without Kubernetes metrics or dstat. Use this when metrics are failing or you only need load-test logs and CSVs. |
...
| `FEED_BASE_URL=https://qa-api.loco.com/fd/ ./scripts/run-test-v6.sh --no-k8s --no-dstat --test 2 --feed-v5-webhome` | Runs only Feed v5 webhome (`FLOW_ID=91`) against the QA feed service. Authentication is generated by the load-test project. |
...
| `./scripts/run-test-v6.sh --k8s --test 2` | Runs the 225.5k-user test and also starts Kubernetes metrics. Use only when `ssh my-machine` and `kubectl` are healthy. |
...
| Region set | Command |
|---|---|
| Full 225.5k current test | `./scripts/run-test-v6.sh --no-k8s --test 2` |
| Active Brazil machine only | `./scripts/run-test-v6.sh --no-k8s --no-dstat --preset brazil-active` |
...
| Philippines | 55k | `load-test-linux-philippines-01..03` |
...
```text
GET /fd/v5/feed/?limit=6&offset=0&type=webhome
```

The load-test repository creates a unique synthetic user JWT and device ID for
each execution. Do not supply a static authorization token. It also validates
that the response status is 200 and the body is at least the configured minimum
size.
...
```bash
FEED_BASE_URL=https://qa-api.loco.com/fd/ \
  ./scripts/run-test-v6.sh --no-k8s --no-dstat --test 2 \
  --feed-v5-webhome --flow-duration 180s
```

Review the resolved flow, target RPS, and feed configuration without SSH:
...
Set `FEED_CACHE_KEY` only for a deliberate cache-key test:

```bash
FEED_BASE_URL=https://qa-api.loco.com/fd/ \
FEED_CACHE_KEY=<explicit-cache-key> \
  ./scripts/run-test-v6.sh --no-k8s --no-dstat --test 2 \
  --feed-v5-webhome
```
...
Sync all default flows for the 225.5k test:
...
For a Feed v5 test, scope collection to the Feed namespace and deployment so
the short test spends its sampling time on the backend under test:

```bash
K8S_SSH_HOST=my-machine \
K8S_NAMESPACES="feed-service" \
K8S_DEPLOYMENTS="feed-service/feed-api-deployment" \
K8S_INTERVAL_SECONDS=5 \
FEED_BASE_URL=https://<qa-origin-load-test-host>/fd/ \
VIETNAM_USERS_K=25 \
  ./scripts/run-test-v6.sh --all-metrics \
  --feed-v5-webhome \
  --machines "load-test-vietnam-01 load-test-vietnam-02" \
  --flow-duration 60s
```

No permanent service-specific collector is required on `my-machine`.
`k8s-cluster-metrics.sh` copies itself to that host, samples
`feed-service/feed-api-deployment`, and downloads the results into the matching
run directory.
...
- Current 225.5k command: `./scripts/run-test-v6.sh --no-k8s --test 2`.
