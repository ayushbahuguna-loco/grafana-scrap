...
STREAM_UID="${STREAM_UID:-708c22e6-040f-493d-b795-d471d528a1e4}"
STREAMER_UID="${STREAMER_UID:-4UYUL1EPDA}"
leaderboard="${leaderboard:-${LEADERBOARD:-550ebc65-7e08-43af-98d5-f26b62c4a94e}}"
PRINT_API_RESPONSE="${PRINT_API_RESPONSE:-true}"
...
#   Philippines:   PHILIPPINES_USERS_K (default 100k) -> load-test-linux-philippines-01..05
...
PHILIPPINES_USERS_K="${PHILIPPINES_USERS_K:-100}"
...
Run flags:
  ...
  --philippines-users-k 25       Override the Philippines regional user target. Default: 100.
...
Load-test authentication:
  JWT_SECRET_KEY                   Required. Loaded from .env and passed to run-direct.sh.
  PRINT_API_RESPONSE               Print API request/response details. Default: true.
  leaderboard                      Leaderboard identifier passed to run-direct.sh.
...
build_remote_load_env_command() {
    ...
    printf -v quoted_value '%q' "$JWT_SECRET_KEY"
    command+=" JWT_SECRET_KEY=$quoted_value"

    printf -v quoted_value '%q' "$PRINT_API_RESPONSE"
    command+=" PRINT_API_RESPONSE=$quoted_value"

    printf -v quoted_value '%q' "$leaderboard"
    command+=" leaderboard=$quoted_value"
...
machine_region_key() {
    case "$1" in
        ...
        philippines-01|philippines-02|philippines-03|philippines-04|philippines-05|load-test-linux-philippines-01|load-test-linux-philippines-02|load-test-linux-philippines-03|load-test-linux-philippines-04|load-test-linux-philippines-05) printf '%s\n' 'philippines' ;;
...
do
    case "$1" in
        ...
        --philippines-users-k)
            if [ "$#" -lt 2 ]; then
                echo "--philippines-users-k requires a value"
                exit 1
            fi
            PHILIPPINES_USERS_K="$2"
            shift 2
            continue
            ;;
        --philippines-users-k=*)
            PHILIPPINES_USERS_K="${1#*=}"
            ;;
        --run-id)
            if [ "$#" -lt 2 ]; then
...
set_bool_from_value PRINT_API_RESPONSE "$PRINT_API_RESPONSE" "PRINT_API_RESPONSE"
...
if ! [[ "$PHILIPPINES_USERS_K" =~ ^[0-9]+([.][0-9]+)?$ ]] || \
    ! awk -v users_k="$PHILIPPINES_USERS_K" 'BEGIN { exit !(users_k > 0) }'
then
    echo "PHILIPPINES_USERS_K must be a positive number: $PHILIPPINES_USERS_K"
    exit 1
fi
...
if [ -z "$JWT_SECRET_KEY" ]; then
    echo "JWT_SECRET_KEY must be set in .env or exported before running"
    exit 1
fi

if [ -z "$leaderboard" ]; then
    echo "leaderboard must not be empty"
    exit 1
fi
...
REMOTE_LOAD_ENV_COMMAND="$(build_remote_load_env_command)"
if [ -n "$FEED_CACHE_KEY" ]; then
...
echo "PrintApiResponse=$PRINT_API_RESPONSE"
...
echo "JwtSecretKeyConfigured=true"
if [ -n "$REGION_MACHINE_COUNT_OVERRIDE" ]; then
...
do
    ...
    (
        ...
        machine_ssh_retry "$machine" "
            ...
            echo 'Leaderboard=$leaderboard' >> \$SUMMARY_FILE
            echo 'PrintApiResponse=$PRINT_API_RESPONSE' >> \$SUMMARY_FILE
